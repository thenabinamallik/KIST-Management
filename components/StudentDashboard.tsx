...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, setUser }) => {
  ...
  const handleTableSave = () => {
    ...
    // Flash a success message or just visual feedback via state
  };
  ...
  const cancelTableEdit = () => {
    if (studentData) {
      setTempRecords([...studentData.records]);
    }
    setEditingIndices(new Set());
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
        ...
        <div className="flex-1 text-center md:text-left space-y-2">
          {isEditing ? (
            <div className="space-y-4 max-w-sm">
              ...
              <div className="space-y-1">
                 <label className="text-xs font-bold text-slate-400 uppercase">Photo URL</label>
                 <input 
                    type="text" 
            ...
            <>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-4">
                <span className="flex items-center space-x-2 text-xs font-bold px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 text-kist-blue rounded-full">
                  <BookOpen size={14} /> <span>Computer Science</span>
                </span>
                <span className="flex items-center space-x-2 text-xs font-bold px-3 py-1.5 bg-amber-50 dark:bg-amber-900/20 text-amber-600 rounded-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700">
           <div className="flex items-center space-x-4">
              <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 rounded-xl"><Award size={24} /></div>
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Current GPA</p>
                <h4 className="text-2xl font-black text-slate-800 dark:text-white">{latestGPA}</h4>
              </div>
           </div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700">
           <div className="flex items-center space-x-4">
              <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 rounded-xl"><Calendar size={24} /></div>
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Attendance</p>
                <h4 className="text-2xl font-black text-slate-800 dark:text-white">{latestAttendance}%</h4>
              </div>
           </div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 hidden lg:block">
           <div className="flex items-center space-x-4">
              <div className="p-3 bg-rose-50 dark:bg-rose-900/20 text-rose-600 rounded-xl"><BookOpen size={24} /></div>
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Credits</p>
                <h4 className="text-2xl font-black text-slate-800 dark:text-white">124</h4>
              </div>
           </div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 hidden lg:block">
           <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <h4 className="text-2xl font-black text-slate-800 dark:text-white">02</h4>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
          <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6">Academic Progress</h3>
      ...
      <section className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center space-x-3">
            {editingIndices.size > 0 && (
              <div className="flex items-center space-x-2 animate-in slide-in-from-right-4">
                <button 
                  ...
                  className="flex items-center space-x-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-200 transition-all text-sm font-semibold"
                >
                  <X size={16} />
                  <span>Cancel</span>
                </button>
                <button 
                  ...
                  className="flex items-center space-x-2 px-4 py-2 bg-kist-blue text-white rounded-xl hover:bg-blue-800 shadow-md shadow-kist-blue/20 transition-all text-sm font-bold"
                >
                  <Save size={16} />
            ...
            <button 
              onClick={toggleSort}
              className="flex items-center space-x-2 px-4 py-2 bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl border border-slate-200 dark:border-slate-700 transition-all text-sm font-semibold"
            >
              <ArrowUpDown size={16} />
              <span className="hidden sm:inline">Sort: {sortOrder === 'asc' ? 'Asc' : 'Desc'}</span>
                ...
                return (
                  <tr key={record.originalIndex} className="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                    <td className="px-6 py-4">
                      {isRowEditing ? (
                        <input 
                          ...
                          onChange={(e) => updateTempRecord(record.originalIndex, 'semester', e.target.value)}
                          className="w-full p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm dark:text-white focus:ring-2 focus:ring-kist-blue outline-none"
                        />
                      ) : (
                    ...
                    <td className="px-6 py-4 text-center">
                      {isRowEditing ? (
                        <input 
                          ...
                          max="4"
                          value={record.gpa}
                          onChange={(e) => updateTempRecord(record.originalIndex, 'gpa', parseFloat(e.target.value))}
                          className="w-20 mx-auto p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-center dark:text-white focus:ring-2 focus:ring-kist-blue outline-none"
                        />
                      ) : (
                        <span className={`inline-block px-3 py-1 rounded-full text-sm font-bold ${
                          record.gpa >= 3.5 ? 'bg-green-50 text-green-600 dark:bg-green-900/20' : 
                          record.gpa >= 2.5 ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/20' : 'bg-red-50 text-red-600 dark:bg-red-900/20'
                        }`}>
                    ...
                    <td className="px-6 py-4 text-center">
                      {isRowEditing ? (
                        <input 
                          ...
                          onChange={(e) => updateTempRecord(record.originalIndex, 'attendance', parseInt(e.target.value))}
                          className="w-20 mx-auto p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-center dark:text-white focus:ring-2 focus:ring-kist-blue outline-none"
                        />
                      ) : (
                        <div className="flex items-center justify-center space-x-2">
                          <div className="flex-1 max-w-[100px] h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden hidden sm:block">
                            <div 
                              className={`h-full rounded-full ${record.attendance >= 85 ? 'bg-emerald-500' : record.attendance >= 75 ? 'bg-amber-500' : 'bg-rose-500'}`}
                    ...
                    <td className="px-6 py-4 text-right pr-10">
                      <button 
                        onClick={() => toggleRowEdit(record.originalIndex)}
                        className={`p-2 rounded-lg transition-all ${
                          isRowEditing 
                            ? 'bg-kist-blue/10 text-kist-blue hover:bg-kist-blue/20' 
                            : 'text-slate-400 hover:text-kist-blue hover:bg-slate-100 dark:hover:bg-slate-700'
                        }`}
                        title={isRowEditing ? "Done editing row" : "Edit row"}
                      >
      ...
      <section className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
        <div className="flex items-center space-x-3 mb-8">
          <div className="p-2 bg-kist-gold/20 text-kist-gold rounded-lg">
            <ChartIcon size={24} />
          </div>
          <div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={currentRecords} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <YAxis 
                ...
                tickLine={false}
                axisLine={false}
                label={{ value: 'GPA', angle: -90, position: 'insideLeft', offset: 10, fill: '#1e3a8a', fontWeight: 'bold' }}
              />
              <YAxis 
                yAxisId="right" 
                orientation="right" 
                ...
                tickLine={false}
                axisLine={false}
                label={{ value: 'Attendance %', angle: 90, position: 'insideRight', offset: 10, fill: '#fbbf24', fontWeight: 'bold' }}
              />
              <Tooltip 
                contentStyle={{ 
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100/50 dark:border-blue-900/20">
            <h4 className="text-sm font-bold text-kist-blue mb-1">GPA Insight</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              Your academic performance shows a {currentRecords.length > 1 && currentRecords[currentRecords.length-1].gpa > currentRecords[0].gpa ? 'positive upward' : 'stable'} trend. Keep maintaining focus on core subjects.
            </p>
          </div>
          <div className="p-4 bg-amber-50/50 dark:bg-amber-900/10 rounded-2xl border border-amber-100/50 dark:border-amber-900/20">
            <h4 className="text-sm font-bold text-amber-600 mb-1">Attendance Policy</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              Maintain at least 75% attendance to remain eligible for end-semester examinations. Your current average is {(currentRecords.reduce((acc, r) => acc + r.attendance, 0) / currentRecords.length).toFixed(1)}%.
            </p>
          </div>
        </div>
      </section>
